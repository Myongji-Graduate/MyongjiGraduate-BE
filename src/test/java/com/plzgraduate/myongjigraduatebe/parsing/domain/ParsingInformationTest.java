...
class ParsingInformationTest {
	...
	void createParsingInformation() {
		...
		//then
		assertThat(parsingInformation).extracting(
			"studentName",
			"studentNumber",
			"major",
			"subMajor",
	...
	void 부전공생_확인() {
		...
		assertThat(parsingInformation).extracting(
			...
			"associatedMajor",
			"studentCategory"
		).contains("정지환", "60181666", "데이터테크놀로지전공", null, "응용소프트웨어전공", StudentCategory.SUB_MAJOR);
	}
	...
	@DisplayName("새 포맷(2026년 이후)에서 입학 정보가 마지막 줄로 이동해도 신입학생은 NORMAL로 파싱된다.")
	@Test
	void 새포맷_신입학_정상파싱() {
		//given
		String parsingText = "출력일자 :  2026/04/04|1/1"
			+ "|미디어·휴먼라이프대학 디지털미디어학부, 박수현(60230544), 현학적 - 재학, 이수 - 5"
			+ "|토익 - 710, 영어교과목면제 - 면제없음, 최종학적변동 - 불일치복학(2026/01/06)"
			+ "|편입생 인정학점 - 교양 0, 전공 0, 자유선택 0, 성경과인간이해 0"
			+ "|교환학생 인정학점 - 공통교양 0, 학문기초교양 0, 일반교양 0, 전공 0, 복수전공학문기초교양 0, 복수전공 0, 융합전공 0, 부전공 0, 자유선택 0"
			+ "|공통교양 17, 핵심교양 12, 학문기초교양 15, 일반교양 8, 전공 35, 복수전공 0, 연계전공 0, 부전공 0, 교직 0, 자유선택 0"
			+ "|총 취득학점 - 87, 총점 - 278.5, 평균평점 - 3.44"
			+ "|이수구분|수강년도/학기|한글코드|과목코드|과목명|학점|등급|중복"
			+ "|공통교양|2024년 1학기|교필157|KMA02157|디지털리터러시의이해|2|P"
			+ "|공통교양|2023년 2학기|교필105|KMA02105|발표와토의|3|B0"
			+ "|공통교양|2023년 1학기|교필127|KMA00101|성서와인간이해|2|B+"
			+ "|공통교양|2023년 1학기|교필123|KMA02123|영어3|2|A0"
			+ "|공통교양|2023년 2학기|교필124|KMA02124|영어4|2|B+"
			+ "|공통교양|2023년 1학기|교필125|KMA02125|영어회화3|1|A+"
			+ "|공통교양|2023년 2학기|교필126|KMA02126|영어회화4|1|A0"
			+ "|공통교양|2023년 1학기|교필101|KMA02101|채플|0.5|P"
			+ "|공통교양|2023년 2학기|교필101|KMA02101|채플|0.5|P"
			+ "|공통교양|2024년 1학기|교필101|KMA02101|채플|0.5|P"
			+ "|공통교양|2024년 2학기|교필101|KMA02101|채플|0.5|P"
			+ "|공통교양|2023년 2학기|교필102|KMA02102|현대사회와기독교윤리|2|B+"
			+ "|전공1단계|2023년 1학기|미디103|HBF01103|커뮤니케이션이론|3|B+"
			+ "|입학 - 신입학(2023/03/02), 사회과학대학(디지털미디어학과)|";
		...
		assertThat(parsingInformation).extracting(
			...
			"major",
			"studentCategory"
		).contains("박수현", "60230544", "디지털미디어학부", StudentCategory.NORMAL);
		assertThat(parsingInformation.getTakenLectureInformation()).hasSize(13);
	}

	@DisplayName("새 포맷(2026년 이후)에서 마지막 줄에 편입학 정보가 있으면 TRANSFER로 파싱된다.")
	@Test
	void 새포맷_편입학_마지막줄에서_감지() {
		//given
		String parsingText = "출력일자 :  2026/04/04|1/1"
			+ "|공과대학 기계공학과, 홍길동(60230001), 현학적 - 재학, 이수 - 3"
			...
			+ "|이수구분|수강년도/학기|한글코드|과목코드|과목명|학점|등급|중복"
			+ "|공통교양|2023년 1학기|교필101|KMA02101|채플|0.5|P"
			+ "|전공1단계|2023년 1학기|기계101|JEP01101|기계공학개론|3|A0"
			+ "|입학 - 편입학(2023/03/02), 공과대학(기계공학과)|";
		...
		assertThat(parsingInformation).extracting(
			...
			"major",
			"studentCategory"
		).contains("홍길동", "60230001", "기계공학과", StudentCategory.TRANSFER);
	}

	@DisplayName("이전 포맷에서 splitText[2]에 편입학 정보가 있으면 여전히 TRANSFER로 파싱된다.")
	@Test
	void 이전포맷_편입학_정상파싱() {
		//given
		String parsingText = "출력일자 :  2022/11/14|1/1"
			+ "|공과대학 기계공학과, 홍길동(60190001), 현학적 - 재학, 이수 - 7, 입학 - 편입학(2019/03/04)"
			+ "|토익 - 700, 영어교과목면제 - 면제없음, 최종학적변동 - 일반복학(2021/01/06)"
			+ "|편입생 인정학점 - 교양 0, 전공 0, 자유선택 0, 성경과인간이해 0"
			+ "|교환학생 인정학점 - 학문기초교양 0, 일반교양 0, 전공 0, 복수전공학문기초교양 0, 복수전공 0, 연계전공 0, 부전공 0, 자유선택 0"
			+ "|공통교양 6, 핵심교양 0, 학문기초교양 0, 일반교양 0, 전공 9, 복수전공 0, 연계전공 0, 부전공 0, 교직 0, 자유선택 0"
			+ "|총 취득학점 - 15, 총점 - 50, 평균평점 - 3.0"
			+ "|이수구분|수강년도/학기|한글코드|과목코드|과목명|학점|등급|중복"
			+ "|공통교양|2019년 1학기|교필101|KMA02101|채플|0.5|P"
			+ "|전공1단계|2019년 1학기|기계101|JEP01101|기계공학개론|3|A0"
			+ "|";
