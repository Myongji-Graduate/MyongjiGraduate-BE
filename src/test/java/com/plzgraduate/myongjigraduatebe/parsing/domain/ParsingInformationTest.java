...
class ParsingInformationTest {
	...
	@DisplayName("편입생의 편입생 인정학점이 정상적으로 추출된다.")
	@Test
	void 편입생_인정학점_추출() {
		String parsingText = "출력일자 :  2023/12/08|1/1"
		...
		assertThat(parsingInformation).extracting(
			...
			"major",
			"transferCredit"
		).contains("최나영", "60195521", "경제학과", TransferCredit.from("51/0/13/0"));
	}

	@DisplayName("비편입생의 편입생 인정학점이 정상적으로 추출된다.")
	@Test
	void 비편입생_인정학점_추출() {
		//given
		String parsingText = "출력일자 :  2023/09/01|1/1"
			...
			+ "|공통교양 17, 핵심교양 12, 학문기초교양 18, 일반교양 5, 전공 63, 복수전공 0, 연계전공 0, 부전공 0, 교직 0, 자유선택 3"
			+ "|총 취득학점 - 118, 총점 - 436, 평균평점 - 3.82"
			+ "|이수구분|수강년도/학기|한글코드|과목코드|과목명|학점|등급|중복|공통교양|2019년 2학기|교필137|KMA02137|4차산업혁명시대의진로선택|2|P";
		//when
		ParsingInformation parsingInformation = ParsingInformation.parsing(parsingText);
		//then
		assertThat(parsingInformation).extracting(
			...
			"major",
			"transferCredit"
		).contains("정지환", "60181666", "데이터테크놀로지전공", TransferCredit.from("0/0/0/0"));
	}
}
