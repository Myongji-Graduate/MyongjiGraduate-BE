...
class ParsingAnonymousServiceTest {

	@InjectMocks
	private ParsingAnonymousService parsingAnonymousService;

	@DisplayName("파싱 텍스트에서 정보를 추출해 anonymous user를 포함한 ParsingAnonymousDto를 반환한다.")
	@Test
	void parseAnonymous() {
		// given
		EnglishLevel englishLevel = EnglishLevel.ENG12;
		KoreanLevel koreanLevel = KoreanLevel.FREE;
		...
		// when
		ParsingAnonymousDto parsingAnonymousDto = parsingAnonymousService.parseAnonymous(
			englishLevel,
			koreanLevel,
			parsingText
		);
		...
		// then
		assertThat(parsingAnonymousDto.getAnonymous()).extracting(
			"authId",
			"name",
			"englishLevel",
			"studentNumber",
