...
/**
 * N택 M의 선택과목이 있는 경우 처리하는 핸들러 클래스 국제통상의 경우 회계원리, 마케팅원론, 재무관리 원론, 인적자원관리, 생산운영관리(폐지), 운영관리(구, 생산운영관리)
 * 5개 중 4개 선택 경영정보의 경우 인적자원관리, 마켓팅원론, 재무관리원론에서 ~18학번까지는 3개 모두 이수, 19학번 이후 택 2 경영의 경우 국제통상원론, 국제경양,
 * 경영정보 중 택1
 **/
@Component
public class OptionalMandatoryMajorHandler implements MandatoryMajorSpecialCaseHandler {
	...
	public boolean isSupport(User user, MajorType majorType) {
		...
		return List.of(BUSINESS, INTERNATIONAL_TRADE)
			.contains(calculatingMajor);
	}
	...
	public MandatorySpecialCaseInformation getMandatorySpecialCaseInformation(User user,
		...
		OptionalMandatory optionalMandatory = OptionalMandatory.from(
			getCalculatingMajor(user, majorType));
		int removedMandatoryTotalCredit = 0;
		boolean isCompletedMandatorySpecialCase = checkCompleteOptionalMandatory(
			takenLectureInventory, mandatoryLectures,
		...
		return MandatorySpecialCaseInformation.of(isCompletedMandatorySpecialCase,
			removedMandatoryTotalCredit);
	}

	private boolean checkCompleteOptionalMandatory(TakenLectureInventory takenLectureInventory,
		Set<Lecture> mandatoryLectures, Set<Lecture> electiveLectures,
		OptionalMandatory optionalMandatory) {
		int chooseNum = optionalMandatory.getChooseNumber();
		/*
		 * 전공과목 Set 에서 전공선택필수과목에 해당되는 과목들을 추출한다.
		 */
		Set<Lecture> optionalMandatoryLectures = mandatoryLectures.stream()
			.filter(
				optionalMandatory.getOptionalMandatoryLectures()::contains)
		...
		for (TakenLecture takenLecture : takenLectureInventory.getTakenLectures()) {
			if (optionalMandatoryLectures.contains(takenLecture.getLecture())
				&& count < chooseNum) {
				count++;
