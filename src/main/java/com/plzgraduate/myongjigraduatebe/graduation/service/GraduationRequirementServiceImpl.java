...
public class GraduationRequirementServiceImpl implements GraduationRequirementService {
  ...
  public BachelorInfoLecturesResponse getBachelorInfoLectures(BachelorInfoRequest bachelorInfoRequest) {
    ...
    classifyGraduationLectures(
        graduationLectures,
        majorMap,
        commonCultureMap,
        coreCultureMap,
        basicAcademicalCultureMap
    );
  ...
  private void classifyGraduationLectures(
        ...
        switch (lectureCategory
          ...
          case "COMMON_CULTURE":
            if (lecture
                .getLectureCode()
                .getCode()
                .equals("KMA00101") && graduationLecture.isMandatory()) {
              lecture.setName(lecture.getName() + "(필수)");
            }
            commonCultureMap
                .computeIfAbsent(lectureCategory.getDetailCategory(), k -> new ArrayList<>())
  ...
  private void divideMajorByMandatory(
      Map<String, List<LectureResponse>> majorMap,
  ...
  private void addEnglishLevelOverThreeToCommonCultureMap(Map<String, List<LectureResponse>> commonCultureMap) {
    ...
    englishThreeFourCode.forEach(lectureCode -> {
      ...
      byLectureCode.ifPresent(lecture -> commonCultureMap
          .get("ENGLISH")
          .add(LectureResponse.from(lecture)));
    });
  }
